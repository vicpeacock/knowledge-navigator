...
# Questo script è nella directory scripts/, quindi PROJECT_ROOT è la directory padre
...
if [ "$BACKEND_READY" = false ]; then
    ...
    if [ -f /tmp/backend.pid ]; then
        ...
        echo "     ✗ Nessun PID trovato"
    fi
    echo "   - Porta 8000:"
    if lsof -ti:8000 > /dev/null 2>&1; then
        echo "     ✓ Porta in uso"
        lsof -ti:8000 | head -1 | xargs ps -p 2>/dev/null || echo "     ⚠️  Processo non trovato"
    else
        echo "     ✗ Porta non in uso"
    fi
    echo ""
